...
When it returns, capture state, evidence, decisions, GAPs, domain updates, and next action before
...
Lodestar: what changed in Goal / State / Domain / GAP after the workflow?
```

Use both together by loading Lodestar first, then invoking the relevant superpowers skill for the
active task. After the task skill completes, record only meaningful state, domain, decision, GAP,
and action updates back into Lodestar.
